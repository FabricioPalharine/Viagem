...
namespace CV.Mobile.Behaviors
{
    public class EventToCommandBehavior : BindableBehavior<View>
    {
        ...
        private void AddEventHandler(EventInfo eventInfo, object item, Action<object, object> action)
        {
            ...
            var actionInvoke = action.GetType()
                .GetRuntimeMethods().First(m => m.Name == "Invoke");
            Expression expressao = Expression.Constant(item,typeof(object));
            if (eventParameters.Length > 1)
                expressao = eventParameters.First();
        ...
        private void OnFired(object sender, object eventArgs)
        {
            if (Command == null)
                return;
