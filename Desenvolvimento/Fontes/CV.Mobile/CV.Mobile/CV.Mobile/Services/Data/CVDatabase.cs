...
{
    public class CVDatabase : IDatabase
    {
        readonly SQLiteAsyncConnection database;

        public CVDatabase(IDependencyService dependencyService)
        {
            var servico = dependencyService.Get<IFileHelper>();
            database = new SQLiteAsyncConnection(servico.GetLocalFilePath("CVBase.db3"), true);
        ...
        public async Task ExcluirItemCompra_IdentificadorGastoCompra(int? identificador)
        {
        ...
        public async Task<List<ViagemAerea>> ListarViagemAerea(CriterioBusca itemBusca)
        {
            ...
            {
                item.Aeroportos = new System.Collections.ObjectModel.ObservableCollection<ViagemAereaAeroporto>(await database.Table<ViagemAereaAeroporto>().Where(d => d.IdentificadorViagemAerea == item.Identificador).ToListAsync());
            }
        ...
        public async Task ExcluirAvaliacaoAtracao_IdentificadorAtracao(int? identificador)
        {
        ...
        public async Task ExcluirAvaliacaoLoja_IdentificadorLoja(int? identificador)
        {
        ...
        public async Task ExcluirHotelAvaliacao_IdentificadorHotel(int? identificador)
        {
        ...
        public async Task ExcluirAvaliacaoAluguel_IdentificadorCarro(int? identificador)
        {
        ...
        public async Task ExcluirRefeicaoPedido_IdentificadorRefeicao(int? identificador)
        {
        ...
        public async Task ExcluirAvaliacaoAerea_IdentificadorViagemAerea(int? identificador)
        {
        ...
        public async Task ExcluirViagemAereaAeroporto_IdentificadorViagemAerea(int? identificador)
        {
        ...
        {
            var queryInicial = (await database.Table<AporteDinheiro>().Where(d => d.IdentificadorUsuario == IdentificadorUsuario).Where(d => d.Moeda == Moeda).Where(d => d.IdentificadorViagem == IdentificadorViagem).ToListAsync())
                .Where(d => !d.DataExclusao.HasValue).Where(d => d.DataAporte < DataInicio).Select(d => new { Data = d.DataAporte.Value, Valor = d.Valor.Value });
            queryInicial = queryInicial.Union(
                (await database.Table<Gasto>().Where(d => d.IdentificadorUsuario == IdentificadorUsuario).Where(d => d.IdentificadorViagem == IdentificadorViagem)
                .Where(d => d.Moeda == Moeda).ToListAsync()).Where(d => !d.DataExclusao.HasValue).Where(d => d.Data < DataInicio).Where(d => d.Especie).Select(d => new { Data = d.Data.Value, Valor = d.Valor.Value * -1 })
                );

            var queryAtual = (await database.Table<AporteDinheiro>().Where(d => d.IdentificadorUsuario == IdentificadorUsuario).Where(d => d.IdentificadorViagem == IdentificadorViagem)
                .Where(d => d.Moeda == Moeda).ToListAsync()).Where(d=>!d.DataExclusao.HasValue).Where(d => d.DataAporte >= DataInicio).Select(d => new { Data = d.DataAporte.Value, Valor = d.Valor.Value, Descricao = "Aporte Moeda" });
            queryAtual = queryAtual.Union((await database.Table<Gasto>().Where(d => d.IdentificadorUsuario == IdentificadorUsuario).Where(d => d.DataExclusao == null).Where(d => d.IdentificadorViagem == IdentificadorViagem)
                .Where(d => d.Moeda == Moeda).Where(d => d.Data >= DataInicio).Where(d => d.Especie).ToListAsync()).Select(d => new { Data = d.Data.Value, Valor = d.Valor.Value * -1, Descricao = d.Descricao }));
        ...
        public async Task<Hotel> ListarHotelAtual()
        {
            var query = database.Table<Hotel>();
            query = query.Where(d=>d.DataExclusao ==null).Where(d => d.DataEntrada != null && d.DataSaidia == null);
            var hotel = await query.FirstOrDefaultAsync();
            if (hotel != null)
            {
                var queryEvento = database.Table<HotelEvento>().Where(d => d.IdentificadorHotel == hotel.Identificador).Where(d=>d.DataExclusao == null).OrderByDescending(d=>d.DataEntrada).Take(2);

                hotel.Eventos = new System.Collections.ObjectModel.ObservableCollection<HotelEvento>(await queryEvento.ToListAsync());
                if (hotel.Movel && hotel.Eventos.Where(d=>d.DataSaida.HasValue).Any())
                {
                    var ultimoEvento = hotel.Eventos.Where(d => d.DataSaida.HasValue).OrderByDescending(d => d.DataSaida).FirstOrDefault();
                    if (ultimoEvento.LatitudeSaida.HasValue && ultimoEvento.LongitudeSaida.HasValue)
                    {
                        hotel.Latitude = ultimoEvento.LatitudeSaida;
                        hotel.Longitude = ultimoEvento.LongitudeSaida;
                    }
                }              
...
");
        ...
        public async Task LimparParticipantes()
        {
            await database.ExecuteAsync("Delete from ParticipanteViagem");
            await database.ExecuteAsync("Delete from Usuario");
        }
    }
}
