...
namespace CV.UI.Web.Controllers.WebAPI
{
    public partial class AtracaoController : BaseApiController
    {
        ...
        public ResultadoConsultaTipo<Atracao> Get(CriterioBusca json)
        {
            ...
            List<Atracao> _itens = biz.ListarAtracao().ToList();
resultado.TotalRegistros = _itens.Count();
            if (json.SortField != null && json.SortField.Any())
                _itens = _itens.AsQueryable().OrderByField<Atracao>(json.SortField, json.SortOrder).ToList();
        ...
        {
            ViagemBusiness biz = new ViagemBusiness();
            Atracao itemAtracao = biz.SelecionarAtracao(id);
        ...
        public ResultadoOperacao Delete(int id)
        {
            ViagemBusiness biz = new ViagemBusiness();
            Atracao itemAtracao = biz.SelecionarAtracao(id);
            biz.ExcluirAtracao(itemAtracao);
            ResultadoOperacao itemResultado = new ResultadoOperacao();
            itemResultado.Sucesso = biz.IsValid();
            itemResultado.Mensagens = biz.RetornarMensagens.ToArray();
