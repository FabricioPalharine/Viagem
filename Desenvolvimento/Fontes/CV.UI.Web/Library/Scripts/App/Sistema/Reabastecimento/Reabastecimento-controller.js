(function () {
	...
	angular
		.module('Sistema')
		.controller('ReabastecimentoCtrl',['$uibModal', 'Error', '$timeout', '$state', '$translate', '$scope', 'Auth', '$rootScope', '$stateParams', '$window', 'i18nService','Carro','Cidade','Reabastecimento', ReabastecimentoCtrl]);

	function ReabastecimentoCtrl($uibModal,  Error, $timeout, $state, $translate, $scope, Auth, $rootScope, $stateParams, $window, i18nService,Carro,Cidade,Reabastecimento) {
		var vm = this;
		...
		vm.delete = function (itemForDelete, indexForDelete, callback) {
			vm.loading = true;
			Reabastecimento.delete({ id: itemForDelete.Identificador }, function (data) {
				callback(data);
				if (data.Sucesso) {
					vm.CarregarDadosWebApi(vm.pagingOptions.pageSize, vm.pagingOptions.currentPage);
        ...
        vm.ActionModalCtrl = function ($uibModalInstance, item, index) {
            ...
            vmAction.edit = function (idToEdit) {
                $uibModalInstance.close();
                $state.go('ReabastecimentoEdicao', { id: idToEdit, filtro: vm.filtroAtualizacao });
            };
...
//
        vm.CarregarDadosWebApi = function (pageSize, page) {
            ...
            Reabastecimento.list({ json: JSON.stringify(vm.filtroAtualizacao) }, function (data) {
                vm.loading = false;
...
//
        vm.gridOptions = {
            data: 'itemReabastecimento.ListaDados',           
            			columnDefs: [
				{field:'Identificador',  displayName: '', cellTemplate: "BotoesGridTemplate.html",  width: 60,},
				{field:'IdentificadorCarro', displayName: $translate.instant('Reabastecimento_IdentificadorCarro'),},
				{field:'IdentificadorCidade', displayName: $translate.instant('Reabastecimento_IdentificadorCidade'),},
				{field:'Latitude', displayName: $translate.instant('Reabastecimento_Latitude'),cellFilter: 'number:\'8\'' },
				{field:'Longitude', displayName: $translate.instant('Reabastecimento_Longitude'),cellFilter: 'number:\'8\'' },
				{field:'Litro', displayName: $translate.instant('Reabastecimento_Litro'),},
				{field:'QuantidadeReabastecida', displayName: $translate.instant('Reabastecimento_QuantidadeReabastecida'),cellFilter: 'number:\'2\'' },
			],
