(function () {
	...
	angular
		.module('Sistema')
		.controller('PaisCtrl',['$uibModal', 'Error', '$timeout', '$state', '$translate', '$scope', 'Auth', '$rootScope', '$stateParams', '$window', 'i18nService','Pais', PaisCtrl]);

	function PaisCtrl($uibModal,  Error, $timeout, $state, $translate, $scope, Auth, $rootScope, $stateParams, $window, i18nService,Pais) {
		var vm = this;
		...
		vm.delete = function (itemForDelete, indexForDelete, callback) {
			vm.loading = true;
			Pais.delete({ id: itemForDelete.Identificador }, function (data) {
				callback(data);
				if (data.Sucesso) {
					vm.CarregarDadosWebApi(vm.pagingOptions.pageSize, vm.pagingOptions.currentPage);
        ...
        vm.ActionModalCtrl = function ($uibModalInstance, item, index) {
            ...
            vmAction.edit = function (idToEdit) {
                $uibModalInstance.close();
                $state.go('PaisEdicao', { id: idToEdit, filtro: vm.filtroAtualizacao });
            };
...
//
        vm.CarregarDadosWebApi = function (pageSize, page) {
            ...
            Pais.list({ json: JSON.stringify(vm.filtroAtualizacao) }, function (data) {
                vm.loading = false;
...
//
        vm.gridOptions = {
            data: 'itemPais.ListaDados',           
            			columnDefs: [
				{field:'Identificador',  displayName: '', cellTemplate: "BotoesGridTemplate.html",  width: 60,},
				{field:'Sigla', displayName: $translate.instant('Pais_Sigla'),},
				{field:'Nome', displayName: $translate.instant('Pais_Nome'),},
			],
