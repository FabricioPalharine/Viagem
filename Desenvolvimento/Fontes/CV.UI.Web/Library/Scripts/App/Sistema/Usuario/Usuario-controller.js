(function () {
	...
	angular
		.module('Sistema')
		.controller('UsuarioCtrl',['$uibModal', 'Error', '$timeout', '$state', '$translate', '$scope', 'Auth', '$rootScope', '$stateParams', '$window', 'i18nService','Usuario', UsuarioCtrl]);

	function UsuarioCtrl($uibModal,  Error, $timeout, $state, $translate, $scope, Auth, $rootScope, $stateParams, $window, i18nService,Usuario) {
		var vm = this;
		...
		vm.delete = function (itemForDelete, indexForDelete, callback) {
			vm.loading = true;
			Usuario.delete({ id: itemForDelete.Identificador }, function (data) {
				callback(data);
				if (data.Sucesso) {
					vm.CarregarDadosWebApi(vm.pagingOptions.pageSize, vm.pagingOptions.currentPage);
        ...
        vm.ActionModalCtrl = function ($uibModalInstance, item, index) {
            ...
            vmAction.edit = function (idToEdit) {
                $uibModalInstance.close();
                $state.go('UsuarioEdicao', { id: idToEdit, filtro: vm.filtroAtualizacao });
            };
...
//
        vm.CarregarDadosWebApi = function (pageSize, page) {
            ...
            Usuario.list({ json: JSON.stringify(vm.filtroAtualizacao) }, function (data) {
                vm.loading = false;
...
//
        vm.gridOptions = {
            data: 'itemUsuario.ListaDados',           
            			columnDefs: [
				{field:'Identificador',  displayName: '', cellTemplate: "BotoesGridTemplate.html",  width: 60,},
				{field:'EMail', displayName: $translate.instant('Usuario_EMail'),},
				{field:'Nome', displayName: $translate.instant('Usuario_Nome'),},
				{field:'Token', displayName: $translate.instant('Usuario_Token'),},
				{field:'RefreshToken', displayName: $translate.instant('Usuario_RefreshToken'),},
				{field:'DataToken', displayName: $translate.instant('Usuario_DataToken'),cellFilter: 'date:\'dd/MM/yyyy\'' },
				{field:'Lifetime', displayName: $translate.instant('Usuario_Lifetime'),},
				{field:'Codigo', displayName: $translate.instant('Usuario_Codigo'),},
			],
