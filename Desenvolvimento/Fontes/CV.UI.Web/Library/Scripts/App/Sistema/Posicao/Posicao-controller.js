(function () {
	...
	angular
		.module('Sistema')
		.controller('PosicaoCtrl',['$uibModal', 'Error', '$timeout', '$state', '$translate', '$scope', 'Auth', '$rootScope', '$stateParams', '$window', 'i18nService','Usuario','Viagem','Posicao', PosicaoCtrl]);

	function PosicaoCtrl($uibModal,  Error, $timeout, $state, $translate, $scope, Auth, $rootScope, $stateParams, $window, i18nService,Usuario,Viagem,Posicao) {
		var vm = this;
		...
		vm.delete = function (itemForDelete, indexForDelete, callback) {
			vm.loading = true;
			Posicao.delete({ id: itemForDelete.Identificador }, function (data) {
				callback(data);
				if (data.Sucesso) {
					vm.CarregarDadosWebApi(vm.pagingOptions.pageSize, vm.pagingOptions.currentPage);
        ...
        vm.ActionModalCtrl = function ($uibModalInstance, item, index) {
            ...
            vmAction.edit = function (idToEdit) {
                $uibModalInstance.close();
                $state.go('PosicaoEdicao', { id: idToEdit, filtro: vm.filtroAtualizacao });
            };
...
//
        vm.CarregarDadosWebApi = function (pageSize, page) {
            ...
            Posicao.list({ json: JSON.stringify(vm.filtroAtualizacao) }, function (data) {
                vm.loading = false;
...
//
        vm.gridOptions = {
            data: 'itemPosicao.ListaDados',           
            			columnDefs: [
				{field:'Identificador',  displayName: '', cellTemplate: "BotoesGridTemplate.html",  width: 60,},
				{field:'IdentificadorViagem', displayName: $translate.instant('Posicao_IdentificadorViagem'),},
				{field:'IdentificadorUsuario', displayName: $translate.instant('Posicao_IdentificadorUsuario'),},
				{field:'Latitude', displayName: $translate.instant('Posicao_Latitude'),cellFilter: 'number:\'8\'' },
				{field:'Longitude', displayName: $translate.instant('Posicao_Longitude'),cellFilter: 'number:\'8\'' },
				{field:'DataGMT', displayName: $translate.instant('Posicao_DataGMT'),cellFilter: 'date:\'dd/MM/yyyy\'' },
				{field:'Velocidade', displayName: $translate.instant('Posicao_Velocidade'),cellFilter: 'number:\'2\'' },
				{field:'DataLocal', displayName: $translate.instant('Posicao_DataLocal'),cellFilter: 'date:\'dd/MM/yyyy\'' },
			],
