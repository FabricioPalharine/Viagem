(function () {
	...
	angular
		.module('Sistema')
		.controller('RequisicaoAmizadeCtrl',['$uibModal', 'Error', '$timeout', '$state', '$translate', '$scope', 'Auth', '$rootScope', '$stateParams', '$window', 'i18nService','Usuario','RequisicaoAmizade', RequisicaoAmizadeCtrl]);

	function RequisicaoAmizadeCtrl($uibModal,  Error, $timeout, $state, $translate, $scope, Auth, $rootScope, $stateParams, $window, i18nService,Usuario,RequisicaoAmizade) {
		var vm = this;
		...
		vm.delete = function (itemForDelete, indexForDelete, callback) {
			vm.loading = true;
			RequisicaoAmizade.delete({ id: itemForDelete.Identificador }, function (data) {
				callback(data);
				if (data.Sucesso) {
					vm.CarregarDadosWebApi(vm.pagingOptions.pageSize, vm.pagingOptions.currentPage);
        ...
        vm.ActionModalCtrl = function ($uibModalInstance, item, index) {
            ...
            vmAction.edit = function (idToEdit) {
                $uibModalInstance.close();
                $state.go('RequisicaoAmizadeEdicao', { id: idToEdit, filtro: vm.filtroAtualizacao });
            };
...
//
        vm.CarregarDadosWebApi = function (pageSize, page) {
            ...
            RequisicaoAmizade.list({ json: JSON.stringify(vm.filtroAtualizacao) }, function (data) {
                vm.loading = false;
...
//
        vm.gridOptions = {
            data: 'itemRequisicaoAmizade.ListaDados',           
            			columnDefs: [
				{field:'Identificador',  displayName: '', cellTemplate: "BotoesGridTemplate.html",  width: 60,},
				{field:'IdentificadorUsuario', displayName: $translate.instant('RequisicaoAmizade_IdentificadorUsuario'),},
				{field:'IdentificadorUsuarioRequisitado', displayName: $translate.instant('RequisicaoAmizade_IdentificadorUsuarioRequisitado'),},
				{field:'EMail', displayName: $translate.instant('RequisicaoAmizade_EMail'),},
				{field:'Status', displayName: $translate.instant('RequisicaoAmizade_Status'),},
			],
