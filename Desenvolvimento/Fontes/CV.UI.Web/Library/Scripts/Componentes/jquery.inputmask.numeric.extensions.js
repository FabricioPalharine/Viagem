	...
	$.extend($.inputmask.defaults.aliases, {
		'decimal': {
			...
			onKeyDown: function (e, buffer, opts) {
				...
				if (e.keyCode == opts.keyCode.DELETE || e.keyCode == opts.keyCode.BACKSPACE) {
					opts.postFormat(buffer, 0, true, opts);
					input._valueSet(buffer.join(''));
					return true;
				}
			},
			onBlur: function (buffer, opts) {
				var teste = 1;
				var $input = $(this), input = this;
				if (opts.radixPoint != "" && opts.digits != "0") {
					...
					if (radixPosition != -1) {
						...
						buffer[buffer.length] = opts.radixPoint;
						radixPosition = $.inArray(opts.radixPoint, buffer);
						var masksets = $input.data('_inputmask')['masksets'];
						var activeMasksetIndex = $input.data('_inputmask')['activeMasksetIndex'];
			...
			definitions: {
				'~': { //real number
					validator: function (chrs, buffer, pos, strict, opts) {
						...
						var escapedGroupSeparator = $.inputmask.escapeRegex.call(this, opts.groupSeparator);
						if (!strict && opts.integerDigits != "" && opts.integerDigits != "*" && chrs != opts.radixPoint) {
							var currentBufferStr = buffer.join(''), strippedBufferStr = currentBufferStr.replace(new RegExp(escapedGroupSeparator, "g"), ""),
                groupOffset = currentBufferStr.length - strippedBufferStr.length;
							var radixPosition = $.inArray(opts.radixPoint, buffer);
							if (radixPosition > -1) {

								if ((radixPosition - groupOffset) >= parseInt(opts.integerDigits) && pos <= radixPosition)
									return false;
							}
							else {
								var UltimaPosicao = 0;
								if (buffer[buffer.length - 1] == "")
									UltimaPosicao = 1;
								if ((buffer.length - groupOffset - UltimaPosicao) >= parseInt(opts.integerDigits))
									return false;
							}
						}

						if (!strict && opts.digits != "" && opts.digits != "0") {
							var radixPosition = $.inArray(opts.radixPoint, buffer);
							var UltimaPosicao = 0;
							if (buffer[buffer.length - 1] == "")
								UltimaPosicao = 1;
							if (radixPosition > -1 && pos > radixPosition && pos != buffer.length - 1 && (buffer.length - 1 - radixPosition - UltimaPosicao) == parseInt(opts.digits)) {
								buffer = buffer.slice(0, buffer.length - 1 - UltimaPosicao);
							}
						}
