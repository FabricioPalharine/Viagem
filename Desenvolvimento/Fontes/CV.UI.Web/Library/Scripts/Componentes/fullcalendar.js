...
function distributeHeight(els, availableHeight, shouldRedistribute) {
	...
	$(flexEls).each(function(i, el) {
		...
		var newHeight = 42;//minOffset - (naturalOffset - naturalHeight); // subtract the margin/padding
...
Grid.mixin({
	...
	segDragMousedown: function(seg, ev) {
		...
		if (view.name == 'year') {
			var td = $(el).closest('div.fc-year-monthly-td');
...
var DayGrid = Grid.extend({
	...
	renderFill: function(type, segs) {
		...
		for (i = 0; i < segs.length; i++) {
			...
			var currentValue = this.rowEls.eq(seg.row).find('.fc-content-skeleton td').eq(seg.leftCol).html();
			this.rowEls.eq(seg.row).find('.fc-content-skeleton td').eq(seg.leftCol).html("");
			seg.el.html(currentValue);
			this.rowEls.eq(seg.row).find('.fc-content-skeleton td').eq(seg.leftCol).append(seg.el);
...
fcViews.year = View.extend({
	...
	buildSkeleton: function(monthsPerRow, showNumbers) {
		...
		s = '<div class="fc-year-main-table fc-border-separate row" style="width:100%;">';
		...
		for (n=0; n<this.nbMonths; n++) {
			...
			s += '<div class="fc-year-monthly-td col-sm-6 col-md-4 col-lg-3 col-xs-12" style="height:310px;' + display + '">';

			s +='<div class="fc-year-monthly-name'+(monthsRow===0 ? ' fc-first' : '')+'">' +
					'<a >' + htmlEscape(monthName) + '</a>' +
				'</div>';
		...
		this.subTables = this.table.find('div.fc-year-monthly-td');
...
*/
	...
	updateHeight: function() {
	    var calendar = this.calendar; // we poll the calendar for height information
	    if (calendar.options.monthHeight)
	        this.setHeight(calendar.options.monthHeight, calendar.isHeightAuto());
        else if (this.yearColumns > 0) {
