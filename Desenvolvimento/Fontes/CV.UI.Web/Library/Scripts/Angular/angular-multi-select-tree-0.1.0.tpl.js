angular.module('multi-select-tree').run(['$templateCache', function($templateCache) {
  ...
  $templateCache.put('src/multi-select-tree.tpl.html',
    "<div class=\"tree-control form-control\">\n" +
    "\n" +
    ...
    "    </span>\n" +
    "    <span ng-if=\"selectedItems.length > 0\" class=\"selected-items\">\n" +
    "      <span ng-repeat=\"selectedItem in selectedItems\" class=\"selected-item\">{{selectedItem.Descricao}}</span>\n" +
    "        <span class=\"fa fa-angle-down pull-right\"></span>\n" +
    "    </span>\n" +
  ...
  $templateCache.put('src/tree-item.tpl.html',
    "<li>\n" +
    "    <div class=\"item-container\" ng-class=\"{active: item.isActive, selected: item.Selecionado}\"\n" +
    "         ng-click=\"clickSelectItem(item, $event)\" ng-mouseover=\"onMouseOver(item, $event)\">\n" +
    "        <span ng-if=\"showExpand(item)\" class=\"fa fa-angle-right\" ng-class=\"{'trasform-opened': item.isExpanded}\"\n" +
    ...
    "                                         ng-checked=\"item.Selecionado\"/>{{item.Descricao}}\n" +
    "        </div>\n" +
    "    </div>\n" +
    "    <ul ng-repeat=\"child in item.Filhos\" ng-if=\"item.isExpanded\">\n" +
