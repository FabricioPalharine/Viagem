...
namespace CV.Mobile.Droid.Renderer
{
    public class GesturesContentViewRenderer : ViewRenderer<GesturesContentView, Android.Views.View>, GestureDetector.IOnGestureListener, GestureDetector.IOnDoubleTapListener
    {
        ...
        private List<Xamarin.Forms.View> ViewsContaing(Point pt)
        {
            var ret = new List<Xamarin.Forms.View>();
            return ViewsContainingImpl(pt, ret, ViewGroup).OrderBy(x => x.Bounds.Width * x.Bounds.Height).ToList();
        }

        private IEnumerable<Xamarin.Forms.View> ViewsContainingImpl(Point pt, List<Xamarin.Forms.View> views, ViewGroup root)
        {
            for (var i = 0; i < root.ChildCount; i++)
            {
                ...
                if (ver == null || !(ver.Element is Xamarin.Forms.View) || !bounds.Contains(Convert.ToInt32(pt.X), Convert.ToInt32(pt.Y)))
                    continue;
                views.Add(ver.Element as Xamarin.Forms.View);
