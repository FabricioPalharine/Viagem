            ...
            using (ConsultaRepository data = new ConsultaRepository())
            {
                ...
                foreach (var itemPosicao in ListaPosicoes.OrderBy(d=>d.DataLocal))
                {
                    if (itemPosicaoAnterior != null)
                    {
                        ...
                        if (itemPosicao.Velocidade < 12 && !ListaIntervaloDeslocamento.Union(ListaIntervaloCarro).Where(d=>d.Partida <= itemPosicao.DataLocal && d.Chegada >= itemPosicao.DataLocal).Any())
                        {
                        ...
                        {
                            DistanciaOutros += DistanciaPontos;
                        }
                    }
